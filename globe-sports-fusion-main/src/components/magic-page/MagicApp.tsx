...
const MagicApp = () => {
  ...
  return (
    <div className="iphone-bottom-pad min-h-[100dvh] pb-[calc(env(safe-area-inset-bottom)+96px)] safe-top safe-x relative overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 bg-background/95 backdrop-blur-xl border-b-2 border-primary/40 shadow-[0_6px_24px_rgba(0,0,0,0.5)]">
        <header className="iphone-tight w-full px-4 pt-[max(env(safe-area-inset-top),8px)] pb-3 flex items-center justify-center">
          {/* Tap logo to open settings — cadre élargi pour matcher la largeur du module Magic Combiné */}
          <button
            onClick={() => setShowSettings((s) => !s)}
            className="tap relative w-full max-w-[428px] mx-auto rounded-2xl overflow-hidden glass-strong holo-scan border border-primary/40 shadow-holo flex items-center justify-center gap-4 px-4 py-3"
      ...
      <main className={`iphone-tight px-4 pt-[calc(env(safe-area-inset-top)+148px)] pb-[max(env(safe-area-inset-bottom),16px)] max-w-[428px] mx-auto w-full relative ${tab !== "coach" && !showSettings ? "module-globe-bg" : ""}`}>
        {showSettings ? (
          <SettingsView
          ...
          >
            ...
            {tab === "coach" && <VoiceCoach context={coachContext} predictions={predictions} />}
